...
function submit() {
    form.post(`/convite/co-producao/${props.token}`);
}
    ...
    <div class="space-y-6">
        ...
        <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div class="flex items-center gap-4 border-b border-zinc-100 p-5 dark:border-zinc-800 sm:p-6">
                ...
                <div class="min-w-0">
                    <p class="text-xs font-medium uppercase tracking-wide text-[var(--color-primary)]">Co-produção</p>
                    <h1 class="truncate text-lg font-bold text-zinc-900 dark:text-white">{{ product.name }}</h1>
                    <p class="text-sm text-zinc-500">
                        Comissão de {{ invite.commission_percent }}% (líquido)
                    </p>
                </div>
            </div>
            ...
            <form class="space-y-4 p-5 sm:p-6" @submit.prevent="submit">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400">
                    Use o e-mail <strong>{{ invite.email }}</strong>. Se você já é membro da equipe ou infoprodutor,
                    <a :href="login_url" class="font-medium text-[var(--color-primary)] hover:underline">entre na sua conta</a>
                    para aceitar sem criar senha nova.
                </p>
                ...
                <div>
                    ...
                    <input v-model="form.email" type="email" required readonly :class="inputClass + ' opacity-80'" />
                ...
                <Button type="submit" variant="primary" class="w-full py-3" :disabled="form.processing">
                    {{ form.processing ? 'Aceitando…' : 'Aceitar e criar conta' }}
                </Button>
            </form>
        </div>
    </div>
